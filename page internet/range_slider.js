function RangeSlider(n, minValue, maxValue, startValues, parentId) {
	...
	// Mouse start position on left button push (Y is not useful because thumbs move only horizontally)
	var mouseStartX = 0;
	...
	var thumbElts = [];
	// Array containing the labels linked to the thumbs : textContent is the current thumb's position and it follows thumb movement
	var thumbLabelElts = [];
	...
	for (i = 0; i < 2; i++) {
		(function(j){
			minmaxLabelElts.push(document.createElement("span"));
	...
	var giveElementsSize = function() {
		...
		thumbWidth = 0.02 * refWidth;
		thumbHeight = 0.6 * refHeight;
		labelWidth = 0.05 * refWidth;
		labelHeight = 0.1 * refHeight;
		trackWidth = 0.8 * refWidth;
		// Track's definitive size
		trackElt.style.width = 0.8 * refWidth + "px";
		trackElt.style.height = 0.2 * refHeight + "px";
	...
	var placeElements = function() {
		...
		// Place the thumbs along the track to their starting position given by thumbValues
		// Place the labels above the corresponding thumbs
		for (i = 0; i < nbThumbs; i++)  {
			(function(j){
				// Center vertically the thumb in the parent container (won't move vertically)
				thumbElts[j].style.top = (refHeight - thumbElts[j].offsetHeight)/2 + "px";
				// Place horizontally the thumb and its label to their starting position on the track relatively to starting value
				thumbElts[j].style.left = trackElt.offsetLeft + (thumbValues[j] * trackWidth) - thumbWidth/2 + "px";
			
				// Place vertically the label above its thumb
				thumbLabelElts[j].style.top = 0.05*refHeight + "px";
				// Place horizontally the label centered on its thumb
				thumbLabelElts[j].style.left = trackElt.offsetLeft + (thumbValues[j] * trackWidth) - labelWidth/2 + "px";
			})(i);
		}
		
		// Place the min and max values labels at the extremities of the track, vertically centered
	...
	var moveThumb = function(evt) {
		// If movingThumbIndex is equal to -1, it means that no thumb is selected and thus the movements of the mouse mustn't affect them
		if (movingThumbIndex !== -1) {
			// Update positions
			var xShift = mouseStartX - evt.clientX;
			thumbElts[movingThumbIndex].style.left = thumbElts[movingThumbIndex].offsetLeft + xShift + "px";
			thumbLabelElts[movingThumbIndex].style.left = thumbLabelElts[movingThumbIndex].offsetLeft + xShift + "px";
			
			// Update label values and threshold values
			var newValue = xShift / trackWidth;
			thumbValues[movingThumbIndex] = newValue;
			thumbLabelElts[movingThumbIndex] = newValue;
	...
	var releaseThumb = function() {
		// -1 in this variable indicates that no thumb is selected and thus none of them should be moving anymore
		movingThumbIndex = -1;
	};
	...
	// Add click handlers to parent container too, to avoid movement from looking weird when the pointer leaves the thumb
	containerElt.onmousedown = moveThumb;
